...
/**
* Perform all the steps necessary to burn the FW into the IC
* @param path name of FW file to load from ums
* @param force if 1, the flashing procedure will be forced and executed regardless the additional info, otherwise the FW in the file will be burnt only if it is newer than the one running in the IC
* @param keep_cx if 1, the CX area will be loaded and burnt otherwise will be skipped and the area will be untouched
* @return OK if success or an error code which specify the type of error encountered
*/
int flashProcedure_from_ums(const char *path, int force, int keep_cx)
{
	Firmware fw;
	struct file *fp;
	mm_segment_t old_fs;
	...
	if (IS_ERR(fp)) {
		logError(1,  "%s: failed to open %s.\n",
				__func__, path);
		res = -ENOENT;
		goto open_err;
	}

	orig_size = fp->f_path.dentry->d_inode->i_size;
	if (orig_size > 0) {
		orig_data = kzalloc(orig_size, GFP_KERNEL);
		nread = vfs_read(fp, (char __user *)orig_data,
			orig_size, &fp->f_pos);
		logError(1, "%s Reading Fw file %s: %d bytes\n", tag,
				__func__, path, orig_size);
...
int flash_erase_unlock(void)
{
	...
	u8 cmd[6] =
	    { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_UNLOCK_CODE0,
...
int flash_full_erase(void)
{
	...
	u8 cmd1[6] =
	    { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_CODE0 + 1, 0x00 };
...
int flash_erase_page_by_page(ErasePage keep_cx)
{
	...
	u8 cmd1[6] =
	    { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_CODE0 + 1, 0x00 };
	u8 cmd[6] =
	    { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_CODE0, 0xA0 };
	u8 cmd2[9] =
	    { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x01, 0x28, 0xFF, 0xFF, 0xFF,
		0xFF
	};
	u8 mask[4] = { 0 };
...
int start_flash_dma(void)
{
	...
	u8 cmd[6] = { FLASH_CMD_WRITE_REGISTER, 0x20, 0x00, 0x00, FLASH_DMA_CODE0, FLASH_DMA_CODE1 };
