...
static void fts_init_touchmode_data(void)
{
	...
	/* finger hysteresis */
	xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_DEF_VALUE] = 20;
	xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][SET_CUR_VALUE] = 20;
	xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_CUR_VALUE] = 20;
	xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_MAX_VALUE] = 50;
	xiaomi_touch_interfaces.touch_mode[Touch_UP_THRESHOLD][GET_MIN_VALUE] = 20;

	/*  Tolerance */
	xiaomi_touch_interfaces.touch_mode[Touch_Tolerance][GET_DEF_VALUE] = 5;
	xiaomi_touch_interfaces.touch_mode[Touch_Tolerance][GET_MAX_VALUE] = 30;
...
int fts_p_sensor_cmd(int input)
{
	...
	if (input) {
		...
		ret = fts_write(hover_on, sizeof(hover_on));
	} else {
		ret = fts_write(cmd_off, sizeof(cmd_off));
		ret = fts_write(hover_off, sizeof(hover_off));
	}
	if (ret < OK) {
